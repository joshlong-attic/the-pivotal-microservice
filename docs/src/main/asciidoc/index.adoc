...
.A complete, working REST service in `hello-world/src/main/java/demo/Application.java`
[source,java]
----
include::{code-root}hello-world/src/main/java/demo/Application.java[]
----
...
To make this work, I needed to have a  Maven dependency on my `CLASSPATH`: `org.springframework.boot:spring-boot-starter-web:1.1.8.RELEASE`. This service takes advantage of Spring Boot's Maven plugin which makes running this service as simple as `mvn clean install && java -jar target/hello-world.jar`
...
An application written for  Cloud Foundry takes advantage of open, commodity backing services like http://postgresql.org[PostgreSQL], http://rabbitmq.org[RabbitMQ], and http://redis.io[Redis] so that developers can acheive identical stacks in local, staging and production environments without being tied to proprietary APIs like Amazon's SQS or Google App Engine's task queue.
...
[source,text]
----
include::{code-root}portable/src/main/resources/application.properties[]
----
.this file - `application.properties` - will be loaded when the Spring Boot application starts up.
...
----
.There are two `DataSource`s configured in this application, but Spring profiles can be used to selectively _activate_ the right one based on the environment.

The Spring Cloud project provides PaaS connectors that make this sort of indirection a snap to setup on Cloud Foundry or Heroku, letting you write PaaS agnostic code if you don't want to pursue that same indirection through configuration properties. Because of this flexibility, Spring applications can even be made to run on legacy environments like IBM WebSphere, Oracle WebLogic or JBoss WildFly. Now _that's_ portable!
...
The {12f}/config[12 Factor applications guidance on configuration] suggests that things that may change between deploys (staging, productino, developer environments, etc.) should be externalzed from the application. Let's distinguish the two types of configuration: application internal configuration and external configuration. Application internal configuration - such as the wiring of the components in the application and the
